...
var debug=false;
var xShift = int(random(-100,100));
var yShift= int(random(-100,100));
var deltaTotalColumns= int(random(0,20));
var order= int(random(0,2));
var orderFunc=0;
var currentTimeCells;
...
function drawClock() {
    ...
    currentTimeCells = str(currentDate)+str(currentTime)
    // print()

    // Define the total number of columns
	let totalColumns = currentTimeCells.length+deltaTotalColumns;
    ...
    for (let y = 0; y < rows; y++) {
        for (let x = 0; x < cols; x++) {
            ...
            let posY = y * cellSizeY + cellSizeY / 2;  // Calculate Y position for text   
                // Order
                if (order==0){
                    orderFunc = abs((x+xShift+y+yShift))%currentTimeCells.length;
                }
                else if (order==1){
                    orderFunc = abs((x+xShift))%currentTimeCells.length;
                }   
                else if (order==2){
                    orderFunc = abs((x*y))%currentTimeCells.length;
                }   

                drawBinaryGrid(posX-cellSizeX/2,posY-cellSizeY/2,cellSizeX,cellSizeY, str(currentTimeCells[orderFunc]));
...
function keyPressed() {
    ...
    // Add/Remove from deltaTotalColumns
    if (key === '+') {
        deltaTotalColumns += 1; // Move left
        print("deltaTotalColumns: "+deltaTotalColumns)
    } else if (key === '-') {
        deltaTotalColumns -= 1; // Move right
        print("deltaTotalColumns: "+deltaTotalColumns)
    }

    // Save render
    if (key === 's') {
        saveCanvas("c2p.clock."+currentTimeCells+".png");
        print("deltaTotalColumns: "+deltaTotalColumns)
    }
}
